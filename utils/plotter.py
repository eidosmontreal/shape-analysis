...
def plot_all_iterations(
    log_dir: str,
    csv_name: str = "train_iterations.csv",
    fname: str = "training_plot.png",
    init_epoch: int = 0,
    last_epoch: int = -1,
) -> NoReturn:
    """
...
def plot_multiple_directories(
    log_dirs: List[str],
    csv_name: str = "train.csv",
    col: str = "loss",
    fname: str = "summary.png",
    window_size: int = 5,
    ...
    for idx, log_dir in enumerate(log_dirs):
        with open(join(log_dir, "args.yaml"), "r") as f:
    ...
    plt.legend(labels)
    plt.ylabel(col + f" (moving avg. over {window_size} epochs)")
    plt.title(title)
    plt.savefig(fname)
    plt.close()
...
def plot_train_test(
    ...
    for ax, log_dir in zip(axes, log_dirs):
        ...
        if last_epoch != -1:
            df = df[df["epoch"] < last_epoch]
        train_plot = ax.plot(moving_average(df["train_loss_mean"], window_size=window_size), color="red", label="Train loss")
        ax.plot(moving_average(df["test_loss_mean"], window_size=window_size), color="black", label="Test loss")
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Loss (averaged)")
        plt.grid()

        ax2 = ax.twinx()
        test_plot = ax2.plot(
            moving_average(df["acc_mean"], window_size=window_size),
            color="blue",
            label="Test",
        )
        ax2.set_ylabel("Test accuracy (averaged)")

        ax.legend(train_plot + test_plot, ["Loss", "Accuracy"], loc="best")
        plt.title(f"{args['model']} trained on {args['dataset']}")
    fig.suptitle(title)
    plt.savefig(os.path.join(log_dir, fname))
    plt.close()
