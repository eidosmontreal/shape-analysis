...
def test_compute_face_area_and_angle():
    ...
    area0 = 0.5 * (norm(normal1) + norm(normal2) + norm(normal3))
    area1 = 0.5 * (norm(normal1) + norm(normal3) + norm(normal4))
    area2 = 0.5 * (norm(normal1) + norm(normal2) + norm(normal4))
    area3 = 0.5 * (norm(normal2) + norm(normal3) + norm(normal4))
    ...
    area, angle = metric_conv.compute_face_area_and_angle(pos, faces)

    assert (torch.abs(answer_area - area) < 1e-6).sum() == 4 and (torch.abs(answer_angle - angle) < 1e-6).sum() == 4
