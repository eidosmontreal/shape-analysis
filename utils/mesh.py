...
def mesh2obj(vertices: np.array, faces: np.array, fname: str='mesh.obj',shift:int=1) -> NoReturn:
    ...
    for v in vertices:
        print('v {} {} {}'.format(v[0],v[1],v[2]),file=mesh_obj)
    for f in faces:
        print('f {} {} {}'.format(f[0]+shift,f[1]+shift,f[2]+shift),file=mesh_obj)
    mesh_obj.close()
...
def pcl2ply(vertices: np.array, fname: str='pcl.ply') -> NoReturn:
    ...
    print('element vertex {}'.format(len(vertices.squeeze())),file=ply)
    ...
    for v in vertices.squeeze():
        print('{} {} {}'.format(v[0],v[1],v[2]),file=ply)
    ply.close()

def mesh2ply(vertices: np.array, faces: np.array ,weights: np.array, weights_to_colours:bool=True,fname:str='mesh.ply') -> NoReturn:
    """
    ...
    print('ply',file=ply)
    print('format ascii 1.0',file=ply)
    print('element vertex {}'.format(len(vertices)),file=ply)
    print('property float x',file=ply)
    print('property float y',file=ply)
    print('property float z',file=ply)
    ...
    print('element face {}'.format(len(faces)),file=ply)
    ...
    for (v,c) in zip(vertices,colours):
        print('{} {} {} {} {} {}'.format(v[0],v[1],v[2],int(c[0]),int(c[1]),int(c[2])),file=ply)
    for f in faces:
        print('3 {} {} {}'.format(int(f[0]),int(f[1]),int(f[2])),file=ply)
    ply.close()
